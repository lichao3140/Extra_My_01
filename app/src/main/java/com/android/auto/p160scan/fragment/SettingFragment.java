...
import com.android.auto.p160scan.barcodescanner.BarcodeScanner;
...
public class SettingFragment extends PreferenceFragment implements Preference.OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        addPreferencesFromResource(R.xml.myadvancesetting);
    ...
    private void updateState() {
        key_prefix.setSummary(Variable.getInstance(getActivity()).GetPrefix());
        key_suffix.setSummary(Variable.getInstance(getActivity()).GetSuffix());
        key_trimleft.setSummary(Variable.getInstance(getActivity()).GetTrimleft());
        key_trimright.setSummary(Variable.getInstance(getActivity()).GetTrimright());
        key_interval_time.setSummary(Variable.getInstance(getActivity()).GetIntervalTime());
        key_filter_character.setSummary(Variable.getInstance(getActivity()).GetFilterCharacter());
        key_outtime.setSummary(Variable.getInstance(getActivity()).GetOutTime());
        m_key_broadcast.setSummary(m_key_broadcast.getEntry());
        key_char.setSummary(key_char.getEntry());
        key_interval_outtime.setSummary(Variable.getInstance(getActivity()).GetIntervalOutTime());
    }
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference) {
        ...
        if (key.equals(ConstantUtil.key_notice)
            ...
            if (BarcodeScanner.id == 0x6603) {
                ...
                // showPassswordDialog1(SettingFragment.this.getString(R.string.iscan_enter_password));
            ...
            showEditDialog(preference, key_title, key, preference.getSummary().toString());
        }
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        final String key = preference.getKey();
        if (key.equals(ConstantUtil.key_broadcast) || key.equals("key_charset")) {
            ListPreference listPreference = (ListPreference) preference;
            CharSequence[] entries = listPreference.getEntries();
            int index = listPreference.findIndexOfValue((String) newValue);
            listPreference.setSummary("" + entries[index]);
            ScanLog.getInstance(getActivity()).LOGD("output:" + entries[index]);

        }
        if (key.equals(ConstantUtil.key_continus_scan)) {
            if (!key_continus_scan.isChecked()) {
                ScanLog.getInstance(getActivity()).LOGD(" manual:Open continus scan");
            } else {
                ScanLog.getInstance(getActivity()).LOGD(" manual:Close continus scan");
            }
        }
        if (key.equals(ConstantUtil.key_scan_stop)) {
            if (!key_scan_stop.isChecked()) {
                ScanLog.getInstance(getActivity()).LOGD(" manual:Open  saving model");
            } else {
                ScanLog.getInstance(getActivity()).LOGD(" manual:Close saving scan");
            }
        }

        if (key.equals(ConstantUtil.key_deletect)) {
            if (!m_key_deletect.isChecked()) {
                ScanLog.getInstance(getActivity()).LOGD(" manual:delet char");
            } else {
                ScanLog.getInstance(getActivity()).LOGD(" manual:no delet char");
            }
        }
    ...
    public void showEditDialog(final Preference preference, final String key_title, final String key, final String key_text) {

        LinearLayout inputLayout;
        if (key.equals("key_prefix") || key.equals("key_suffix") || key.equals("key_filter_character")) {
            inputLayout = (LinearLayout) getActivity().getLayoutInflater().inflate(R.layout.input1, null);
        } else if (key.equals(ConstantUtil.key_advance_password)) {
            inputLayout = (LinearLayout) getActivity().getLayoutInflater().inflate(R.layout.input2, null);
        } else {
            inputLayout = (LinearLayout) getActivity().getLayoutInflater().inflate(R.layout.input, null);
        }
                        ...
                        new android.content.DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                if (key.equals("key_prefix")
                                    ...
                                    Variable.getInstance(getActivity()).setCookieVariables(key, mCurrentFormat.getText().toString());
                                    preference.setSummary(mCurrentFormat.getText().toString());
                                    StartService();
                                } else if (key.equals(ConstantUtil.key_advance_password)) {
                                    if (mCurrentFormat.getText().toString().equals(Variable.getInstance(getActivity()).GetAdvancePassword())) {
                                        showPassswordDialog(getActivity().getString(R.string.iscan_advance_newpassword));
                                    } else {
        ...
        new AlertDialog.Builder(getActivity())
                ...
                .setPositiveButton(
                        ...
                        new android.content.DialogInterface.OnClickListener() {
                            @Override
                            public void onClick(DialogInterface dialog, int which) {
                                Variable.getInstance(getActivity()).SetAdvancePassword(mCurrentFormat.getText().toString());
                                StartService();
                            }
