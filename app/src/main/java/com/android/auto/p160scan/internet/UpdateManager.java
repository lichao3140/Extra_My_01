...
public class UpdateManager {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			switch (msg.what) {
				...
				case DOWN_ERROR:
					// ÏÂÔØapkÊ§°Ü
					Toast.makeText(mContext, "ÏÂÔØÐÂ°æ±¾Ê§°Ü", 1).show();

					break;
				case GET_UNDATAINFO_ERROR:
					// ·þÎñÆ÷³¬Ê±
					Toast.makeText(mContext, "»ñÈ¡·þÎñÆ÷¸üÐÂÐÅÏ¢Ê§°Ü", 1).show();
					break;
	...
	private int getAPPVersionCodeFromAPP(Context ctx) {
		...
		try {
			...
			String appVersionName = info.versionName; // °æ±¾Ãû
			currentVersionCode = info.versionCode; // °æ±¾ºÅ
