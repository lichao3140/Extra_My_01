...
import com.android.auto.p160scan.barcodescanner.OnScanListener;
import com.android.auto.p160scan.barcodescanner.ScanCallBack;
...
//			msg.arg1 = (int) (decodeEndTime - decodeStartTime);
	...
	private int decodeTime(long decodeEndTime, long decodeStartTime) {
		int result_time = (int) (decodeEndTime - decodeStartTime);
		if (result_time <= 300) {
			return result_time;
		} else if (result_time <= 500) {
			result_time = result_time - 100;
		} else if (result_time <= 700) {
			result_time = result_time - 200;
		} else if (result_time <= 1000) {
			result_time = result_time - 300;
		} else {
			result_time = result_time - 400;
		}
		return result_time;
	}
}
